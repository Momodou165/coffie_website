	...
	if (userId == undefined) {
		...
		if (installed_date){
			...
			if (installed_date > aug_21_chat_update) {
				...
				.then(async (option) => {
					if (option === "Open") vscode.commands.executeCommand("blackbox.showChat")
				})
			}
		}
	}
				...
				thisWebview.webview.onDidReceiveMessage(async (data) => {
					...
					if (data.command) {
						if (data.command === "sendMessage") {
							...
							open('https://calendly.com/blackboxapp/30min')
						}
					}
				})
		...
		`
			...
			<body>
				...
				<l class='text-description'>
					...
					5. Schedule an Onboarding Call<br><br>
			...
			<body>
				...
				(function() {
					...
					function showStarterText(){
						...
						const $el = $('<div class="code-block-input chatMessage recieved starter-text"><div class="by">BLACKBOX</div><div class="chatMessage-text">Hello, Blackbox AI Chat can answer coding questions. Below are recommendations to get the best out of BLACKBOX</div><ul>'+recommendations+'</ul><div>NEW FEATURE RELEASES: </div><ul>'+new_features+'</ul></div>')
